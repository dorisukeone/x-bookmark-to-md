...
- `export_error` — a fixed error-reason code (e.g. `connection_failed`, `message_timeout`, `message_port_closed`, `zip_download_failed`, `zip_generation_large`) and a fixed export-stage code (`connect`, `connection`, `message_timeout`, `extract`, `convert`, `zip`, `download`, `popup_closed` if you close the popup before an in-progress export finishes, or `download_<reason>` where `<reason>` is one of Chrome's fixed `downloads.InterruptReason` enum values such as `FILE_TOO_LARGE` or `ACCESS_DENIED`); never the raw error text.
